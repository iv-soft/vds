...
vds::expected<void> vds::dht::network::service::start(
  ...
  CHECK_EXPECTED(sp->get<db_model>()->async_transaction([this, sp, port, dev_network, &final_tasks](database_transaction& t) -> expected<void> {
    CHECK_EXPECTED(this->client_.load_keys(sp, t));
