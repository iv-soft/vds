  ...
  lambda_holder_t<vds::async_task<vds::expected<std::shared_ptr<stream_output_async<uint8_t>>>>, std::shared_ptr<tcp_network_socket>> new_connection)
{
  ...
  this->wait_accept_task_ = std::thread(
    [this, sp, new_connection_ = std::move(new_connection)]() {
    ...
    for (;;) {
      ...
      if ((WSAEvents.lNetworkEvents & FD_ACCEPT)
        ...
        if (INVALID_SOCKET != socket) {
          sp->get<logger>()->trace(ThisModule, "Connection from %s", network_service::to_string(client_address).c_str());

          if (!(*sp->get<network_service>())->associate(socket).has_error()) {
            auto s = _tcp_network_socket::from_handle(socket);
            new_connection_(s).then([sp, s](vds::expected<std::shared_ptr<stream_output_async<uint8_t>>> result) {
              if (result.has_error()) {
          ...
          const auto error = WSAGetLastError();
          sp->get<logger>()->trace(ThisModule, "Accept error %s", std::system_error(error, std::system_category(), "listen socket").what());
        }
      }
    }
  });
